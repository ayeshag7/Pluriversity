...
import { FollowerPointerCard } from "./FollowerPointerCard"; // Ensure the path is correct
      ...
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-4 py-4">
        <a href="https://flowbite.com/" className="flex items-center space-x-3 rtl:space-x-reverse">
          <span className="self-center text-xl md:text-2xl font-semibold whitespace-nowrap text-white">
            The <span className="ml-2">Pluriversity</span> <span className="text-2xl md:text-3xl ml-2">لاھور</span>
          </span>
          <Image src="/Images/logo-f.png" height={64} width={64} alt="Pluriversity Logo" />
        </a>
        <button
          onClick={() => setHidden(!hidden)}
          data-collapse-toggle="navbar-default"
          type="button"
          className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-[#89BE63] hover:text-[white] rounded-lg md:hidden hover:bg-[#89BE63] focus:outline-none focus:ring-2 focus:ring-gray-200"
        ...
        <div className={`${hidden ? "hidden" : ""} w-full md:block md:w-auto`} id="navbar-default">
          <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-600 rounded-lg bg-[#0A0708] md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-[#0A0708]">
            <li>
              <Link
                href="/"
